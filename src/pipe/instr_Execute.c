 ...
 **************************************************************************/
 ...
 comb_logic_t execute_instr(x_instr_impl_t *in, m_instr_impl_t *out) {
    ...
    uint64_t alu_operand_b = (in->X_sigs.valb_sel == 0) ? in->val_b : in->val_imm;

    uint8_t nzcv = 0;
    bool cond_val = false;
    alu(in->val_a, alu_operand_b, in->val_hw, in->ALU_op, in->X_sigs.set_flags, in->cond, &out->val_ex, &cond_val, guest.proc->NZCV);
