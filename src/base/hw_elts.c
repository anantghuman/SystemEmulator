...
comb_logic_t regfile(uint8_t src1, uint8_t src2, uint8_t dst, uint64_t val_w,
                     bool w_enable, uint64_t *val_a, uint64_t *val_b) {
    // Student TODO
    switch (src1) {
        case 31:
            *val_a = guest.proc->SP;
...
comb_logic_t alu(uint64_t alu_vala, uint64_t alu_valb, uint8_t alu_valhw,
                 alu_op_t ALUop, bool set_flags, cond_t cond, uint64_t *val_e,
                 bool *cond_val, uint8_t *nzcv) {
    uint64_t res = 0xFEEDFACEDEADBEEF;  // An initial value to help
                                        // detect errors.
    bool v = false;                     // Overflow flag indicator.
